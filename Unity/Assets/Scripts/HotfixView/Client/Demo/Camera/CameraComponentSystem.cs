        ...
        private static Vector3 CameraRealPos(this CameraComponent self)
        {
            switch (self.cameraMode)
            {
                case CameraMode.Third:
                    ...
                    if (Physics.Raycast(ray, out RaycastHit hit, 1 + self.mainCamera.nearClipPlane, self.mapMask))
                    {
                ...
                case CameraMode.Free:
                    ...
                    if (Physics.Raycast(ray, out hit, self.cameraNodePos.magnitude - 0.1f, self.mapMask))
                    {
